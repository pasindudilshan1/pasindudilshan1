<div align="center">

# Hi there, I'm Pasindu Premarathna 👋

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&duration=3000&pause=1000&color=00D9FF&center=true&vCenter=true&width=600&lines=.NET+Developer+%7C+Data+Analyst;BSc.+IT+%40+UCSC+%7C+BBM+%40+UoK;Full-Stack+Developer;Data+Engineering+Enthusiast;Always+learning%2C+always+building)](https://git.io/typing-svg)

[![Portfolio](https://img.shields.io/badge/Portfolio-00D9FF?style=for-the-badge&logo=google-chrome&logoColor=white)](https://www.pasindupremarathna.dev)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/pasindu-premarathna-22888b314)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:pasindupremarathna11@gmail.com)
[![DataCamp](https://img.shields.io/badge/DataCamp-03EF62?style=for-the-badge&logo=datacamp&logoColor=white)](https://www.datacamp.com/portfolio/premaraas22017)

</div>

---

## 🚀 About Me

```yaml
name: Pasindu Premarathna
role: .NET Developer & Data Analyst
education: 
  - BSc. Information Technology @ UCSC
  - BBM (Hons) Accounting Information Systems @ University of Kelaniya
experience: 1+ years in software development
location: Colombo, Sri Lanka 🇱🇰
current_role: Software Developer Trainee @ AlgotechSys
focus:
  - C#, ASP.NET MVC, Web API Development
  - Microsoft Azure Cloud Services
  - Data Analytics & Business Intelligence
  - ETL Pipeline Development
  - AI & Automation Solutions
learning:
  - Microsoft Fabric Analytics Engineer
  - SnowPro Associate Certification
  - Advanced Azure Services
  - Data Engineering at Scale
certifications:
  - Certified Associate Data Analyst (DataCamp)
  - Data Analyst in SQL (DataCamp)
  - Certified Python Data Associate (DataCamp)
memberships:
  - The Institute of Analytics (Student Member)
  - CA Sri Lanka (Corporate Level Student)
philosophy: "The best way to predict the future is to create it."
```

💡 .NET Developer with hands-on experience building scalable web applications using **ASP.NET MVC, Web API, and C#**. Skilled in **Microsoft Azure** services and **SQL Server** database management. Strong foundation in **data analytics** with expertise in **Power BI, SQL, and Python**. Currently developing AI-powered automation solutions and ETL pipelines while pursuing dual degrees in IT and Business Analytics. Passionate about transforming data into actionable insights and building reliable, high-quality software solutions.

---

## 💼 What I'm Up To

🏢 **Working** as Software Developer Trainee at AlgotechSys  
🎓 **Studying** Information Technology at UCSC & Accounting Information Systems at UoK  
🔨 **Building** ASP.NET applications and cloud-based solutions on Azure  
📊 **Analyzing** business data with Power BI, SQL, and Python for data-driven insights  
🤖 **Developing** AI automation workflows using N8N and Azure services  
☁️ **Deploying** solutions on Azure, AWS, and Google Cloud platforms  
📚 **Preparing** for Microsoft Fabric Analytics Engineer certification  
🏆 **Achieved** 1st Runner-Up at SDG Youth Challenge 2025  
🤝 **Contributing** to FOSS Community and tech organizations at university  
💻 **Managing** projects using Agile/Scrum with Azure DevOps and Git

---

## 🛠️ Tech Stack

### Languages & Core Technologies
![C#](https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=c-sharp&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white)

### Frameworks & Web Development
![ASP.NET](https://img.shields.io/badge/ASP.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Angular](https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js&logoColor=white)
![Laravel](https://img.shields.io/badge/Laravel-FF2D20?style=for-the-badge&logo=laravel&logoColor=white)

### Cloud & DevOps
![Azure](https://img.shields.io/badge/Microsoft_Azure-0078D4?style=for-the-badge&logo=microsoft-azure&logoColor=white)
![AWS](https://img.shields.io/badge/Amazon_AWS-232F3E?style=for-the-badge&logo=amazon-aws&logoColor=white)
![Google Cloud](https://img.shields.io/badge/Google_Cloud-4285F4?style=for-the-badge&logo=google-cloud&logoColor=white)
![Azure DevOps](https://img.shields.io/badge/Azure_DevOps-0078D7?style=for-the-badge&logo=azure-devops&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)

### Data Analytics & BI
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=for-the-badge&logo=power-bi&logoColor=black)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=for-the-badge&logo=tableau&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white)
![Excel](https://img.shields.io/badge/Microsoft_Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white)

### Data Engineering & Big Data
![Apache Spark](https://img.shields.io/badge/Apache_Spark-E25A1C?style=for-the-badge&logo=apache-spark&logoColor=white)
![Kafka](https://img.shields.io/badge/Apache_Kafka-231F20?style=for-the-badge&logo=apache-kafka&logoColor=white)
![Hadoop](https://img.shields.io/badge/Hadoop-66CCFF?style=for-the-badge&logo=apache-hadoop&logoColor=black)
![Snowflake](https://img.shields.io/badge/Snowflake-29B5E8?style=for-the-badge&logo=snowflake&logoColor=white)
![Databricks](https://img.shields.io/badge/Databricks-FF3621?style=for-the-badge&logo=databricks&logoColor=white)
![Airflow](https://img.shields.io/badge/Airflow-017CEE?style=for-the-badge&logo=apache-airflow&logoColor=white)

### Databases
![SQL Server](https://img.shields.io/badge/SQL_Server-CC2927?style=for-the-badge&logo=microsoft-sql-server&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-3ECF8E?style=for-the-badge&logo=supabase&logoColor=white)

### AI & Automation
![N8N](https://img.shields.io/badge/N8N-EA4B71?style=for-the-badge&logo=n8n&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white)
![Claude AI](https://img.shields.io/badge/Claude_AI-181818?style=for-the-badge&logo=anthropic&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-121212?style=for-the-badge&logo=chainlink&logoColor=white)

### Development Tools
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white)
![Visual Studio](https://img.shields.io/badge/Visual_Studio-5C2D91?style=for-the-badge&logo=visual-studio&logoColor=white)
![IntelliJ IDEA](https://img.shields.io/badge/IntelliJ_IDEA-000000?style=for-the-badge&logo=intellij-idea&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=jupyter&logoColor=white)
![Alteryx](https://img.shields.io/badge/Alteryx-0078C0?style=for-the-badge&logo=alteryx&logoColor=white)

---

## 🎯 Areas of Expertise

<table>
<tr>
<td width="50%" valign="top">

### 💻 Software Development
- ASP.NET MVC & Web API
- C# .NET Framework
- RESTful API Design
- Microservices Architecture
- Azure Cloud Services
- CI/CD with Azure DevOps

</td>
<td width="50%" valign="top">

### 📊 Data Analytics & BI
- Power BI Dashboard Development
- SQL Database Management
- Statistical Analysis
- Data Visualization
- KPI Tracking & Reporting
- Business Intelligence

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🔧 Data Engineering
- ETL Pipeline Development
- Apache Spark & Kafka
- Azure Data Factory
- Snowflake & Databricks
- Data Integration
- Big Data Processing

</td>
<td width="50%" valign="top">

### 🤖 AI & Automation
- N8N Workflow Automation
- RAG Applications
- AI-Powered Solutions
- Process Automation
- Integration Development
- Intelligent Document Processing

</td>
</tr>
</table>

---

## 🔥 Featured Projects

<div align="center">

### ⚡ [AI Developer Productivity Analysis](https://github.com/pasindudilshan1)
**Power BI | SQL | Azure Data Studio**  
Analyzed AI developer productivity using simulated datasets to identify performance factors and cognitive load patterns with Power BI dashboards for visualization.

---

### 🔄 [Department of Energy ETL Pipeline](https://github.com/pasindudilshan1/Powering-Data-for-the-Department-of-Energy---Building-an-ETL-Pipeline)
**Python | ETL | Data Integration**  
Production-ready ETL pipeline for processing and transforming energy department data with automated extraction, cleaning, and loading workflows.

---

### 🤖 [N8N RAG Workflow Automation](https://github.com/pasindudilshan1)
**N8N | Supabase | OpenAI | Google Drive**  
Automated workflow integrating Google Drive, Supabase, and OpenAI for AI-powered question answering with document retrieval.

---

### 🧾 [AI Invoice Processing System](https://github.com/pasindudilshan1)
**N8N | AI | Google Sheets**  
Automated invoice data extraction from PDFs using AI with structured export to Google Sheets for streamlined processing.

---

### 🌐 [Web Scraping Tool with AWS Lambda](https://github.com/pasindudilshan1)
**Python | AWS Lambda | Claude AI**  
Built web scraping tool with GUI to extract website data including SEO metrics, structured content, and analytics with AWS Lambda backend.

---

### 🛒 [SmartCart - Intelligent Shopping System](https://github.com/pasindudilshan1/SmartCart)
**AI | Full-Stack Development**  
AI-powered shopping cart application featuring automated checkout, product recognition, and seamless payment integration.

---

### 📚 [Library Management System](https://github.com/pasindudilshan1/Library_Management_System_Project)
**ASP.NET | SQL Server**  
Complete library management solution with book cataloging, member management, borrowing system, and automated notifications.

---

### 📊 [DataCamp Analytics Portfolio](https://www.datacamp.com/portfolio/premaraas22017)
**Python | SQL | Power BI**  
Analyzing Crime in LA | NYC School Test Scores | Netflix Movies | Motorcycle Sales | Student Mental Health | HR Analytics

</div>

---

## 🏆 Achievements & Certifications

### 🎖️ Awards
- 🥈 **1st Runner-Up** - SDG Youth Challenge 2025 (ICBI 2025, University of Kelaniya)

### 📜 Professional Certifications
✅ **Certified Associate Data Analyst** - DataCamp  
✅ **Data Analyst in SQL** - DataCamp  
✅ **Certified Python Data Associate** - DataCamp  
🔄 **SnowPro Associate: Platform Certification** - In Progress  
🔄 **Microsoft Fabric Analytics Engineer Associate** - In Progress

### 🎓 Professional Qualifications
- **The Institute of Analytics** - Student Member (ID: 2507B17750)
- **Chartered Accountants of Sri Lanka** - Corporate Level Student

---

## 📊 GitHub Analytics

<div align="center">

<img height="180em" src="https://github-readme-stats-git-masterrstaa-rickstaa.vercel.app/api?username=pasindudilshan1&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true&bg_color=1a1b27&title_color=00D9FF&icon_color=00D9FF&text_color=c9d1d9"/>

<img height="180em" src="https://github-readme-stats-git-masterrstaa-rickstaa.vercel.app/api/top-langs/?username=pasindudilshan1&layout=compact&langs_count=8&theme=tokyonight&hide_border=true&bg_color=1a1b27&title_color=00D9FF&text_color=c9d1d9"/>

</div>

<div align="center">

<img src="https://streak-stats.demolab.com/?user=pasindudilshan1&theme=tokyonight&hide_border=true&background=1a1b27&stroke=00D9FF&ring=00D9FF&fire=00D9FF&currStreakLabel=00D9FF" alt="GitHub Streak"/>

</div>

---

## 🏆 GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=pasindudilshan1&theme=tokyonight&no-frame=true&no-bg=false&margin-w=4&row=1&column=7" alt="GitHub Trophies"/>

</div>

---

## 👥 Leadership & Community

<table>
<tr>
<td align="center" width="33%">

### 🎓 AISSA - UoK
**IT Coordinator**  
Organizing Committee

</td>
<td align="center" width="33%">

### 🦁 Leo Club - UoK
**PR & Marketing Coordinator**  
Organizing Committee

</td>
<td align="center" width="33%">

### 💻 FOSS Community
**Active Member**  
University of Kelaniya

</td>
</tr>
</table>

---

## 🌐 Let's Connect!

<div align="center">

I'm always excited to connect with fellow developers, data enthusiasts, and tech professionals!

**Open to:**
- 💼 Full-time opportunities in .NET Development & Data Analytics
- 🤝 Open-source collaborations
- 💡 Tech discussions and knowledge sharing
- 🚀 Freelance projects and consulting

<br>

[![Portfolio](https://img.shields.io/badge/🌐_Portfolio-Visit_My_Website-00D9FF?style=for-the-badge)](https://www.pasindupremarathna.dev)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect_with_me-0077B5?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/pasindu-premarathna-22888b314)
[![Email](https://img.shields.io/badge/Email-Drop_a_message-D14836?style=for-the-badge&logo=gmail)](mailto:pasindupremarathna11@gmail.com)
[![DataCamp](https://img.shields.io/badge/DataCamp-View_Portfolio-03EF62?style=for-the-badge&logo=datacamp)](https://www.datacamp.com/portfolio/premaraas22017)

</div>

---

<div align="center">

### 💭 Quote of the Day

<img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight" alt="Random Dev Quote"/>

---

### 👀 Profile Views

<img src="https://komarev.com/ghpvc/?username=pasindudilshan1&color=00D9FF&style=for-the-badge" alt="Profile Views"/>

---

### ⚡ About Me in Code

```javascript
const pasindu = {
  title: ".NET Developer & Data Analyst",
  company: "AlgotechSys",
  experience: "1+ year",
  education: {
    primary: "BSc. IT @ UCSC",
    secondary: "BBM (Hons) AIS @ University of Kelaniya"
  },
  technologies: {
    backend: ["C#", "ASP.NET MVC", "Web API", "Node.js"],
    frontend: ["React", "Angular", "JavaScript"],
    dataAnalytics: ["Power BI", "Tableau", "SQL", "Python", "Excel"],
    dataEngineering: ["Spark", "Kafka", "Hadoop", "Snowflake", "Databricks"],
    cloud: ["Azure", "AWS", "Google Cloud"],
    databases: ["SQL Server", "MySQL", "PostgreSQL", "MongoDB"],
    automation: ["N8N", "Azure Functions", "Lambda"],
    tools: ["Git", "Azure DevOps", "Docker", "Jupyter", "Alteryx"]
  },
  certifications: [
    "Certified Associate Data Analyst (DataCamp)",
    "Data Analyst in SQL (DataCamp)",
    "Python Data Associate (DataCamp)"
  ],
  currentFocus: [
    "ASP.NET Core Development",
    "Azure Cloud Architecture",
    "ETL Pipeline Engineering",
    "Power BI Dashboard Development",
    "Microsoft Fabric Analytics"
  ],
  achievements: {
    competition: "1st Runner-Up - SDG Youth Challenge 2025",
    memberships: ["Institute of Analytics", "CA Sri Lanka"]
  },
  funFact: "I build production apps by day and automate everything by night! 🚀"
};
```

---

**⭐ From [pasindudilshan1](https://github.com/pasindudilshan1)**

*"The best way to predict the future is to create it." - Building the future, one commit at a time.*

---

### 📌 Quick Stats

![](https://img.shields.io/github/followers/pasindudilshan1?style=social)
![](https://img.shields.io/github/stars/pasindudilshan1?style=social)

</div>
